...
interface SemanticRule {
  id: string
  pattern: RegExp
  validator: (match: RegExpMatchArray, context: ValidationContext) => ValidationError[]
...
export class SemanticValidator {
  private readonly SEMANTIC_RULES: SemanticRule[] = [
    // ✅ Actor Reference Validation (Actor:Id notation)
    {
      ...
      validator: (match, context) => {
        ...
        if (!actorRef.match(/^Actor:[A-Z][a-zA-Z0-9]*$/)) {
          return [{
            ...
            severity: 'error',
            suggestion: `Use Actor:${this.toCamelCase(actorRef)} instead`,
    ...
    // ✅ Flow Step Notation Validation (actor: Actor:Id → "action")
    {
      id: 'FLOW_STEP_NOTATION',
      pattern: /actor:\s*Actor:([A-Z][a-zA-Z0-9]*)\s*→\s*"(.+)"/gi,
      validator: (match, context) => {
    ...
    // ✅ DRY Compliance - Inline Foundational Block Detection
    {
      ...
      pattern: /^(\s*)(intent|context|authority|evaluation):\s*$/gmi,
      validator: (match, context) => {
        const blockType = match[2].toLowerCase()
        const isWithinUsesBlock = context.lines.slice(Math.max(0, context.currentLine - 5), context.currentLine)
    ...
    // ✅ Hierarchical Scope Validation
    {
      ...
      validator: (match, context) => {
        ...
        const validArtifacts = ['Vision', 'Principle', 'Policy', 'Guideline', 'Area', 'Actor', 'Process', 'Procedure', 'Event', 'Result', 'Observation', 'Intent', 'Context', 'Authority', 'Evaluation', 'Indicator']
  ...
  private extractArtifacts(text: string, artifacts: Map<string, ArtifactInfo>): void {
  ...
  private isInFlowSection(lines: string[], currentLine: number): boolean {
    ...
    for (let i = currentLine - 1; i >= 0; i--) {
      ...
      if (line.endsWith(':') && !line.includes('steps') && !line.includes('flow')) {
        return false
      }
    }
    return false
  ...
  private canReference(level: string, area: string, dependency: string, allowedRefs: string[]): boolean {
  ...
  private findDuplicateFoundationalBlocks(text: string): Array<{type: string, line: number, suggestedId: string}> {
    const blocks = []
    const foundationalTypes = ['intent', 'context', 'authority', 'evaluation']
  ...
  private generateImprovementWarnings(text: string, artifacts: Map<string, ArtifactInfo>): ValidationWarning[] {
    const warnings: ValidationWarning[] = []
    
    // Check for missing foundational artifacts
    const missingFoundational = ['Intent', 'Context', 'Authority', 'Evaluation'].filter(type => 
